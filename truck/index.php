...
if ($slug !== '' && preg_match('/^[a-z0-9-]+$/i', $slug)) {
    try {
        $stmt = db()->prepare(
            'SELECT id, badge, image_url, tags, title, slug, content, duration, 
              vehicle_type, distance_in_km, price, rating, data_categories,
              meta_description, page_keywords
             FROM packages
             WHERE active = 1 AND type = :type AND slug = :slug
             LIMIT 1'
        );
        $stmt->execute([':type' => 'truck', ':slug' => $slug]);
...
$contactFormPrefix = 'truck';
$contactTitle = 'Contact Us';
$contactSubtitle = 'Share your details and we will get back to you shortly.';
$contactApiUrl = '/api/contact.php';
$contactServiceType = 'truck';
